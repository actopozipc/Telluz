...
namespace ki
{
    class CalculateData
    {
        ...
        public List<Countrystats> GenerateForEachCountry()
        {
            List<string> laender = GetCountries(); //Liste mit allen Ländern
            Task[] tasks = new Task[laender.Count];
            ...
            for (int i = 0; i < laender.Count; i++)
            {
                        ...
                        c.country = laender[i];
                        c.ListWithCategoriesWithYearsAndValues = Generate(laender[i]);
                        countrystats.Add(c);
        ...
        public List<CategoriesWithYearsAndValues> Generate(string country)
        {
            ...
            countrystats.ListWithCategoriesWithYearsAndValues = getCategoriesWithValuesAndYears(country); //Werte mit Jahren
        ...
        public List<CategoriesWithYearsAndValues> getCategoriesWithValuesAndYears(string country)
        {
            ...
            foreach (var item in vs)
            {
                ...
                kmjw.category = item;
                command.CommandText = $"SELECT year, ROUND(values, 20) FROM input_data JOIN category c on input_data.cat_id = c.cat_id JOIN country_or_area coa on input_data.coa_id = coa.coa_id WHERE c.name = 'Urban population' AND coa.name = 'Austria';";
